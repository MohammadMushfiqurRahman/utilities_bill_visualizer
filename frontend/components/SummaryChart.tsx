...
const getColorForVendor = (vendorName: string): string => {
  ...
  if (lowerVendor.includes('electric') || lowerVendor.includes('power') || lowerVendor.includes('pge')) return vendorColors.electric;
  if (lowerVendor.includes('water')) return vendorColors.water;
  if (lowerVendor.includes('gas')) return vendorColors.gas;
  if (lowerVendor.includes('garbage') || lowerVendor.includes('recology') || lowerVendor.includes('waste')) return vendorColors.garbage;
  if (lowerVendor.includes('internet') || lowerVendor.includes('comcast') || lowerVendor.includes('verizon')) return vendorColors.internet;
...
const SummaryChart: React.FC<SummaryChartProps> = ({ bills }) => {
  ...
  if (!data.chartData.length) {
    return <div className="text-center p-8 text-slate-500">No data to display in chart.</div>;
  }
    ...
    <div style={{ width: '100%', height: 350 }}>
      <ResponsiveContainer>
        ...
        >
          ...
          <XAxis dataKey="month" stroke="rgb(100 116 139)" fontSize={12} tickLine={false} axisLine={false} />
          <YAxis stroke="rgb(100 116 139)" fontSize={12} tickLine={false} axisLine={false} tickFormatter={(value) => `$${value}`} />
          <Tooltip 
          ...
          {data.vendorKeys.map(vendor => (
            <Bar key={vendor} dataKey={vendor} stackId="a" fill={getColorForVendor(vendor)} radius={[4, 4, 0, 0]} />
          ))}
        </BarChart>
      </ResponsiveContainer>
    </div>
