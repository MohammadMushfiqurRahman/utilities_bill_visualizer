...
const billSchema = {
  ...
  properties: {
    ...
    apartment: {
        type: Type.STRING,
        description: "The apartment, unit number, or service address if present (e.g., 'Apt 101', 'Unit B', '123 Main St'). If not present, use the account holder's name or another unique identifier.",
    },
    billDate: {
      type: Type.STRING,
    ...
    usage: {
      ...
      properties: {
        value: {
          type: Type.NUMBER,
          description: "The numerical value of the primary consumption (e.g., 545.0, 7.5). If no usage is listed (e.g. for garbage), this can be 0.",
        },
        unit: {
          type: Type.STRING,
          description: "The unit of consumption (e.g., 'kWh', 'Gallons', 'Therms'). If no unit, provide a descriptor like 'Service'.",
        },
      },
      required: ['value', 'unit'],
    },
    ...
    breakdown: {
        type: Type.ARRAY,
        description: "A list of line items from the bill, including their description and amount. For example, 'Previous Balance', 'Electric Charges', 'Taxes'. If not present, return an empty array.",
        items: {
            type: Type.OBJECT,
            properties: {
                description: {
  ...
  required: ['vendorName', 'apartment', 'billDate', 'dueDate', 'totalAmount', 'usage', 'accountNumber'],
};
...
export async function extractBillDataFromPdf(pdfBase64: string) {
  try {
    ...
    try {
        ...
        throw new Error("Could not extract data from the PDF. The AI model returned a malformed response.");
    }
    ...
    const validBills = billsArray.filter(bill => 
        bill && bill.vendorName && bill.billDate && bill.totalAmount && bill.apartment
    );
    
    if (validBills.length === 0) {
        throw new Error("Extracted data is missing required fields like vendor, date, amount, or apartment/address.");
    }
    ...
    if (error instanceof Error && error.message.includes("400")) {
        throw new Error("The provided file does not appear to be a valid or readable utility bill. Please try another file.");
    }
    // Re-throw specific errors to be caught in the UI
    if (error instanceof Error) {
        throw error;
    }
    throw new Error("Could not extract data from the PDF. The AI model might have had trouble understanding this document's format.");
  }
}
